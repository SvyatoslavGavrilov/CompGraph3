//***************************************************************************************
// Lab1App.cpp by Frank Luna (C) 2015 All Rights Reserved.
//***************************************************************************************
...
struct RenderItem
{
	...
	// Because we have an object cbuffer for each Lab1FrameResource, we have to apply the
	// update to each Lab1FrameResource.  Thus, when we modify obect data we should set 
...
class Lab1App : public D3DApp
{
public:
    Lab1App(HINSTANCE hInstance);
    Lab1App(const Lab1App& rhs) = delete;
    Lab1App& operator=(const Lab1App& rhs) = delete;
    ~Lab1App();
...
private:
    ...
    std::vector<std::unique_ptr<Lab1FrameResource>> mFrameResources;
    Lab1FrameResource* mCurrFrameResource = nullptr;
...
Lab1App::Lab1App(HINSTANCE hInstance)
    : D3DApp(hInstance)
{
}
...
void Lab1App::MoveBackFwd(float step) {
	XMFLOAT3 newPos;
...
void Lab1App::MoveLeftRight(float step) {
	XMFLOAT3 newPos;
...
void Lab1App::MoveUpDown(float step) {
	XMFLOAT3 newPos;
...
void Lab1App::Update(const GameTimer& gt)
{
	...
	ImGui::Text("Frame time: %.3f ms", gt.DeltaTime() * 1000.0f);
	ImGui::Text("Camera speed: %.2f", cam.GetSpeed());
	ImGui::Text("Controls: WASD/QE, Shift sprint, hold LMB to look");
	ImGui::End();
...
void Lab1App::OnMouseDown(WPARAM btnState, int x, int y)
{
    mLastMousePos.x = x;
    mLastMousePos.y = y;
...
void Lab1App::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void Lab1App::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void Lab1App::OnKeyPressed(const GameTimer& gt, WPARAM key)
{
...
void Lab1App::OnKeyReleased(const GameTimer& gt, WPARAM key)
{
...
std::wstring Lab1App::GetCamSpeed()
{
	return std::to_wstring(cam.GetSpeed());
}
 
void Lab1App::UpdateCamera(const GameTimer& gt)
{
	...
	XMStoreFloat3(&mEyePos, cam.GetPosition());
}




void Lab1App::AnimateMaterials(const GameTimer& gt)
{
	
}

void Lab1App::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for(auto& e : mAllRitems)
	{
		...
		if(e->NumFramesDirty > 0)
		{
			...
			// Next Lab1FrameResource need to be updated too.
			e->NumFramesDirty--;
		}
	}
}

void Lab1App::UpdateLightCBs(const GameTimer& gt)
{
    ...
    for (auto& light : mLights)
    {
        ...
        PassShadowConstants shadowConstants{};
        ...
        if (light.type == 2)
        {
            ...
            if (XMVector3LengthSq(lightDir).m128_f32[0] < 1e-6f)
            {
                lightDir = XMVectorSet(0.0f, -1.0f, 0.0f, 0.0f);
            }

            if (XMVector3LengthSq(lightPos).m128_f32[0] < 1e-6f)
            {
                lightPos = XMVectorSet(0.0f, 100.0f, 0.0f, 1.0f);
            }

            XMMATRIX lightView = XMMatrixLookToLH(lightPos, lightDir, up);
            XMStoreFloat4x4(&light.LightView, XMMatrixTranspose(lightView));

            const float sceneExtent = 150.0f;
            XMMATRIX lightProj = XMMatrixOrthographicLH(sceneExtent, sceneExtent, 1.0f, 500.0f);
            XMStoreFloat4x4(&light.LightProj, XMMatrixTranspose(lightProj));

            XMMATRIX lightViewProj = lightView * lightProj;
            XMStoreFloat4x4(&light.LightViewProj, XMMatrixTranspose(lightViewProj));
            shadowConstants.LightViewProj = light.LightViewProj;
        }
        else
        {
            XMStoreFloat4x4(&shadowConstants.LightViewProj, XMMatrixIdentity());
        }

        if (light.type == 0)
        {
            ambientColor = XMFLOAT4(light.Color.x * light.Strength,
                light.Color.y * light.Strength,
                light.Color.z * light.Strength,
                1.0f);
        }

        lightConstants.light = light;
        currLightCB->CopyData(light.LightCBIndex, lightConstants);
        currShadowCB->CopyData(light.LightCBIndex, shadowConstants);
    }

    mMainPassCB.AmbientLight = ambientColor;
}

void Lab1App::UpdateMaterialCBs(const GameTimer& gt)
{
	...
	for(auto& e : mMaterials)
	{
		...
		// data changes, it needs to be updated for each Lab1FrameResource.
...
void Lab1App::UpdateMainPassCB(const GameTimer& gt)
{
...
void Lab1App::LoadAllTextures()
{
    mTextures.clear();
    TexOffsets.clear();
    LoadTexture("textures/white1x1");
    LoadTexture("textures/default_nmap");
}

void Lab1App::LoadTexture(const std::string& name)
{
...
void Lab1App::BuildRootSignature()
{
	...
	CD3DX12_ROOT_PARAMETER quadRootParameter[1];
	quadRootParameter[0].InitAsDescriptorTable(1, &texTable, D3D12_SHADER_VISIBILITY_ALL);

	CD3DX12_STATIC_SAMPLER_DESC staticSampler(0, D3D12_FILTER_MIN_MAG_MIP_LINEAR);

	CD3DX12_ROOT_SIGNATURE_DESC quadrootSigDesc(
		1, quadRootParameter,
		1, &staticSampler,
		D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT
	);
...
void Lab1App::BuildLightingRootSignature()
{
...
void Lab1App::BuildShadowPassRootSignature()
{
...
//quads
void Lab1App::BuildScreenQuadGeometry()
{
	struct Vertex {
		XMFLOAT3 Pos;
		XMFLOAT2 TexC;
	...
	float quadW = 0.25f * 4.0f; // размер quad'а (NDC: 0.5 единиц)
	float quadH = 0.25f * 4.0f;

	std::array<Vertex, 6 * 3> vertices;
	...
	// 3 quad-а: главный экран, и два в углу
	MakeQuad(0, -1.0f, -1.0f);         // Shadow Map 2
	MakeQuad(1, 0.0f, -1.0f);         // ShadowMap 2
...
void Lab1App::CreatePointLight(XMFLOAT3 pos, XMFLOAT3 color, float faloff_start, float faloff_end, float strength)
{
	Light light;
...
void Lab1App::CreateSpotLight(XMFLOAT3 pos, XMFLOAT3 rot, XMFLOAT3 color, float faloff_start, float faloff_end, float strength, float spotpower)
{
	Light light;
	light.LightCBIndex = mLights.size();
...
void Lab1App::BuildLights()
{
    mLights.clear();

    Light directional;
    directional.LightCBIndex = mLights.size();
    directional.Position = { 0.0f, 50.0f, 0.0f };
    directional.Direction = { -0.5f, -1.0f, 0.5f };
    directional.Color = { 1.0f, 1.0f, 1.0f };
    directional.Strength = 2.0f;
    directional.type = 2;
    directional.CastsShadows = 1;
    directional.LightUp = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f);
    XMStoreFloat4x4(&directional.gWorld, XMMatrixTranspose(XMMatrixScaling(100.0f, 100.0f, 100.0f)));
    mLights.push_back(directional);

    Light ambient;
    ambient.LightCBIndex = mLights.size();
    ambient.Color = { 1.0f, 1.0f, 1.0f };
    ambient.Strength = 0.15f;
    ambient.type = 0;
    mLights.push_back(ambient);
}

void Lab1App::SetLightShapes()
{
    for (auto& light : mLights)
    {
        if (light.type == 2)
...
void Lab1App::CreateMaterial(std::string _name, int _CBIndex, int _SRVDiffIndex, int _SRVNMapIndex, XMFLOAT4 _DiffuseAlbedo, XMFLOAT3 _FresnelR0, float _Roughness)
{
...
void Lab1App::BuildShadowMapViews()
{
...
void Lab1App::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void Lab1App::BuildShadersAndInputLayout()
{
...
void Lab1App::BuildCustomMeshGeometry(std::string name, UINT& meshVertexOffset, UINT& meshIndexOffset, UINT& prevVertSize, UINT& prevIndSize, std::vector<Vertex>& vertices, std::vector<std::uint16_t>& indices, MeshGeometry* Geo)
{
...
void Lab1App::BuildShapeGeometry()
{
	...
	BuildCustomMeshGeometry("Madoka", meshVertexOffset, meshIndexOffset, prevVertSize, prevIndSize, vertices, indices, geo.get());
...
void Lab1App::BuildFrameResources()
{
	FlushCommandQueue();
	mFrameResources.clear();
    for(int i = 0; i < gNumFrameResources; ++i)
    {
        mFrameResources.push_back(std::make_unique<Lab1FrameResource>(md3dDevice.Get(),
...
void Lab1App::BuildMaterials()
{
    int diffuseIndex = TexOffsets["textures/white1x1"];
    int normalIndex = TexOffsets["textures/default_nmap"];

    CreateMaterial("plane_red", 0, diffuseIndex, normalIndex,
        XMFLOAT4(1.0f, 0.2f, 0.2f, 1.0f), XMFLOAT3(0.04f, 0.04f, 0.04f), 0.2f);
    CreateMaterial("plane_green", 1, diffuseIndex, normalIndex,
        XMFLOAT4(0.2f, 1.0f, 0.4f, 1.0f), XMFLOAT3(0.04f, 0.04f, 0.04f), 0.2f);
    CreateMaterial("plane_blue", 2, diffuseIndex, normalIndex,
        XMFLOAT4(0.2f, 0.4f, 1.0f, 1.0f), XMFLOAT3(0.04f, 0.04f, 0.04f), 0.2f);
    CreateMaterial("plane_yellow", 3, diffuseIndex, normalIndex,
        XMFLOAT4(1.0f, 0.9f, 0.2f, 1.0f), XMFLOAT3(0.04f, 0.04f, 0.04f), 0.2f);
}

void Lab1App::RenderCustomMesh(std::string unique_name, std::string meshname, std::string materialName, XMFLOAT3 Scale, XMFLOAT3 Rotation, XMFLOAT3 Position)
{
...
void Lab1App::BuildRenderItems()
{
    mAllRitems.clear();
    mOpaqueRitems.clear();

    const float baseWidth = 20.0f;
    const float baseDepth = 30.0f;
    const float scaleX = 0.5f;
    const float scaleZ = 0.5f;
    const float tileWidth = baseWidth * scaleX;
    const float tileDepth = baseDepth * scaleZ;

    const std::array<const char*, 4> materialNames = {
        "plane_red", "plane_green", "plane_blue", "plane_yellow"
    };
    ...
    for (int row = 0; row < 2; ++row)
    {
        for (int col = 0; col < 2; ++col)
        {
            ...
            planeItem->Name = "ground_tile_" + std::to_string(row * 2 + col);

            float offsetX = (col == 0 ? -tileWidth * 0.5f : tileWidth * 0.5f);
            float offsetZ = (row == 0 ? -tileDepth * 0.5f : tileDepth * 0.5f);

            XMStoreFloat4x4(
                &planeItem->World,
                XMMatrixScaling(scaleX, 1.0f, scaleZ) *
                XMMatrixTranslation(offsetX, 0.0f, offsetZ));
            XMStoreFloat4x4(&planeItem->TexTransform, XMMatrixScaling(2.0f, 2.0f, 1.0f));
            planeItem->ObjCBIndex = objIndex++;
            planeItem->Mat = mMaterials[materialNames[row * 2 + col]].get();
            planeItem->Geo = mGeometries["shapeGeo"].get();
            planeItem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
            planeItem->IndexCount = planeItem->Geo->DrawArgs["grid"].IndexCount;
            planeItem->StartIndexLocation = planeItem->Geo->DrawArgs["grid"].StartIndexLocation;
            planeItem->BaseVertexLocation = planeItem->Geo->DrawArgs["grid"].BaseVertexLocation;
    ...
    {
        mOpaqueRitems.push_back(renderItem.get());
    }
...
// NOT USING
void Lab1App::Draw(const GameTimer& gt)
{
...
void Lab1App::DrawSceneToShadowMap()
{
...
void Lab1App::DeferredDraw(const GameTimer& gt)
{
	...
	// Find lights that cast shadows and display their shadow maps
	int shadowMapIndex = 0;
	for (auto& light : mLights)
	{
		if (light.type == 2 || light.type == 3)
		{
			if (light.CastsShadows && shadowMapIndex < 2)
			{
				CD3DX12_GPU_DESCRIPTOR_HANDLE texHandle(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
				texHandle.Offset(light.ShadowMapSrvHeapIndex, mCbvSrvDescriptorSize);

				mCommandList->SetGraphicsRootDescriptorTable(0, texHandle);
				mCommandList->DrawInstanced(6, 1, shadowMapIndex * 6, 0);
				shadowMapIndex++;
			}
		}
	}
...
void Lab1App::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    UINT objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(ObjectConstants));
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> Lab1App::GetStaticSamplers()
{
