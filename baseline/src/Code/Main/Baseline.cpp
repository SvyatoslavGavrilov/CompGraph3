...
struct RenderItem
{
	...
	// Because we have an object cbuffer for each BaselineFrameResource, we have to apply the
	// update to each BaselineFrameResource.  Thus, when we modify obect data we should set 
...
void BaselineApp::OnKeyReleased(const GameTimer& gt, WPARAM key)
{
...
void BaselineApp::UpdateCamera(const GameTimer& gt)
{
...
void BaselineApp::AnimateMaterials(const GameTimer& gt)
{
...
void BaselineApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for(auto& e : mAllRitems)
	{
		...
		if(e->NumFramesDirty > 0)
		{
			...
			// Next BaselineFrameResource need to be updated too.
			e->NumFramesDirty--;
		}
	}
}

void BaselineApp::UpdateLightCBs(const GameTimer& gt)
{
...
void BaselineApp::UpdateMaterialCBs(const GameTimer& gt)
{
	...
	for(auto& e : mMaterials)
	{
		...
		// data changes, it needs to be updated for each BaselineFrameResource.
...
void BaselineApp::CreateGBuffer()
{
	// �������
	const DXGI_FORMAT positionFormat = DXGI_FORMAT_R32G32B32A32_FLOAT;
	const DXGI_FORMAT normalFormat = DXGI_FORMAT_R16G16B16A16_FLOAT;
...
void BaselineApp::LoadAllTextures()
{
    mTextures.clear();
    TexOffsets.clear();
...
void BaselineApp::LoadTexture(const std::string& name)
{
...
void BaselineApp::BuildLightingRootSignature()
{
...
void BaselineApp::BuildShadowPassRootSignature()
{
...
//quads
void BaselineApp::BuildScreenQuadGeometry()
{
	struct Vertex {
		XMFLOAT3 Pos;
		XMFLOAT2 TexC;
...
void BaselineApp::CreatePointLight(XMFLOAT3 pos, XMFLOAT3 color, float faloff_start, float faloff_end, float strength)
{
	Light light;
...
void BaselineApp::CreateSpotLight(XMFLOAT3 pos, XMFLOAT3 rot, XMFLOAT3 color, float faloff_start, float faloff_end, float strength, float spotpower)
{
	Light light;
	light.LightCBIndex = mLights.size();
...
void BaselineApp::SetLightShapes()
{
    for (auto& light : mLights)
    {
        if (light.type == 2)
...
void BaselineApp::CreateMaterial(std::string _name, int _CBIndex, int _SRVDiffIndex, int _SRVNMapIndex, XMFLOAT4 _DiffuseAlbedo, XMFLOAT3 _FresnelR0, float _Roughness)
{
...
void BaselineApp::BuildShadowMapViews()
{
...
void BaselineApp::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void BaselineApp::BuildCustomMeshGeometry(std::string name, UINT& meshVertexOffset, UINT& meshIndexOffset, UINT& prevVertSize, UINT& prevIndSize, std::vector<Vertex>& vertices, std::vector<std::uint16_t>& indices, MeshGeometry* Geo)
{
...
void BaselineApp::BuildShapeGeometry()
{
...
void BaselineApp::BuildMaterials()
{
...
void BaselineApp::RenderCustomMesh(std::string unique_name, std::string meshname, std::string materialName, XMFLOAT3 Scale, XMFLOAT3 Rotation, XMFLOAT3 Position)
{
...
void BaselineApp::DrawSceneToShadowMap()
{
...
void BaselineApp::DeferredDraw(const GameTimer& gt)
{
	auto cmdListAlloc = mCurrFrameResource->CmdListAlloc;
	ThrowIfFailed(cmdListAlloc->Reset());
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> BaselineApp::GetStaticSamplers()
{
