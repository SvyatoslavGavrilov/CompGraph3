...
void TexColumnsApp::Update(const GameTimer& gt)
{
	...
	{
		// Handle rotation for main_1 and side objects (60 degrees per second)
		if (rItem->Name == "main_1" || rItem->Name == "side")
		{
			float rotationSpeed = 60.0f; // degrees per second
			float deltaRotation = XMConvertToRadians(rotationSpeed * gt.DeltaTime());
			
			// Update Y and Z rotation
			rItem->RotationAngle.y += deltaRotation;
			rItem->RotationAngle.z += deltaRotation;
			
			// Keep angles in reasonable range
			if (rItem->RotationAngle.y > XM_2PI) rItem->RotationAngle.y -= XM_2PI;
			if (rItem->RotationAngle.z > XM_2PI) rItem->RotationAngle.z -= XM_2PI;
		...
		else if (rItem->Name == "main" || rItem->Name == "itemL" || rItem->Name == "itemR")
		{
...
{
	CreateMaterial("main_mat",0, TexOffsets["textures/mskin"], TexOffsets["textures/mskin_nm"], XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f), XMFLOAT3(0.05f, 0.05f, 0.05f), 0.3f);
...
void TexColumnsApp::BuildRenderItems()
{
	...
	boxRitem->Mat = mMaterials["main_mat"].get();
	...
	RenderCustomMesh("main_1", "Madoka", "main_mat", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 3.14, 0), XMFLOAT3(0, 3, 0));
	RenderCustomMesh("side", "Madoka", "main_mat", XMFLOAT3(3, 3, 3), XMFLOAT3(0, -3.14 / 2, 0), XMFLOAT3(-10, 3, 30));
	RenderCustomMesh("itemL", "left", "eye", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 3.14, 0), XMFLOAT3());
	RenderCustomMesh("itemR", "right", "eye", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 3.14, 0), XMFLOAT3());
	BuildFrameResources();
